<xsl:transform version="1.0"
	...
	<div class="rp_evidence">
			...
			<xsl:if
				test="count($eviNode//cite:citation/cite:person[@id=$currPerson]) > 0">
				<table class="rp_evi">
					<xsl:for-each
						select="$eviNode//cite:citation/cite:person[@id=$currPerson]/ancestor::cite:source/cite:title">
						<tr>
							<td class="rp_evilink">
								<sup>
									<a>
						...
						<xsl:for-each
							...
							<xsl:if test="$cite != 'Unspecified'">
								<tr>
									<td>
										<div class="rp_eviempty"></div>
									</td>
									<td>
										<div class="rp_evicite">
