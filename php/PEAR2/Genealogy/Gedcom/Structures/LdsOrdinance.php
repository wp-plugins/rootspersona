<?php
/**
 * PEAR2\Genealogy\Gedcom\Structures\LdsOrdinance
 *
 * PHP version 5
 *
 ...
 * @version   SVN: $Id: LdsOrdinance.php 291 2010-03-30 19:38:34Z ed4becky $
 ...
 * @link      http://svn.php.net/repository/PEAR2/PEAR2_Genealogy_Gedcom
 */
class LdsOrdinance extends EntityAbstract
{
    // TODO add code
    var $_TYPES = array(
    'BAPL' => '',
    ...
    {
        $events = array();
        foreach (LdsOrdinance::_TYPES as $tag) {
            $off = 0;
            while (($i1=parent::findTag($tree, $tag, $off))!==false) {
                $event = new LdsOrdinance();
                $event->Code = $tag;
                $tmp = LdsOrdinance::_TYPES;
                $event->Type = $tmp[$tag];
